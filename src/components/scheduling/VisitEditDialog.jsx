...
export default function VisitEditDialog({
  ...
  const handleInputChange = (field, value) => {
    ...
    setErrors((currentValue) => ({ ...currentValue, [field]: null }));
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSubmitError(error.message || 'Visit could not be updated.');
    }
  };

  const currentStatus = visit?.status || 'SCHEDULED';
  const statusOptions = getAllowedVisitStatuses(currentStatus).map((status) => ({
    value: status,
      ...
      <form onSubmit={handleSubmit}>
        ...
        {formData.status === 'VACANT' ? (
          <p style={{ fontSize: '12px', color: 'var(--color-text-secondary)', margin: '0 0 12px 0' }}>
            Vacant visits cannot keep a caregiver assignment.
          </p>
        ) : null}
