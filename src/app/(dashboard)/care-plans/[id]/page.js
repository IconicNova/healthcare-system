...
export default function CarePlanDetailPage({ params }) {
  ...
  const handleCloseGenerateModal = () => {
    ...
    setGenerateResult(null);
    setGenerateForm({ startDate: '', endDate: '', staffId: '' });
  };
  ...
  const handleGenerateVisits = async () => {
    ...
    try {
      const response = await fetch(`/api/care-plans/${params.id}/generate-visits`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          startDate: generateForm.startDate,
          endDate: generateForm.endDate,
          staffId: generateForm.staffId || null,
        }),
      });
      ...
      alert(error.message || 'Failed to generate visits');
    } finally {
  ...
  const carePlanStatus = getCarePlanStatusMeta(carePlan.status);
  const hasAutoGeneratedServices = (carePlan.services || []).some(
    (service) => !['AS_NEEDED', 'CUSTOM'].includes(service.frequency)
  );
  const canGenerateVisits = isActiveCarePlanStatus(carePlan.status) && hasAutoGeneratedServices;
  const staffOptions = [
    { value: '', label: 'Use care plan primary staff' },
    ...staff.map((member) => ({
      value: member.id,
    ...
    <div>
      ...
      <div style={{ marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <Button variant="secondary" onClick={handleOpenGenerateModal} disabled={!canGenerateVisits}>
              <Plus size={16} />
              Generate Visits
            </Button>
            {isActiveCarePlanStatus(carePlan.status) && !hasAutoGeneratedServices && (
              <div style={{ fontSize: '12px', color: 'var(--color-text-secondary)', alignSelf: 'center' }}>
                This care plan only has manual services, so visit generation is not available.
              </div>
            )}
      ...
      <div style={{ marginBottom: '24px' }}>
        <span style={{
          ...
          fontSize: '13px',
          fontWeight: 500,
          backgroundColor: carePlanStatus.variant === 'success' ? 'var(--color-success-light)' : 'var(--color-gray-100)',
          color: carePlanStatus.variant === 'success' ? 'var(--color-success)' : 'var(--color-text-secondary)',
        }}>
          {carePlanStatus.label}
        </span>
      </div>
      ...
      <Modal isOpen={showGenerateModal} onClose={handleCloseGenerateModal} title="Generate Visits" size="lg">
        <div style={{ display: 'grid', gap: '16px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <Input
              ...
              onChange={(event) => setGenerateForm((current) => ({ ...current, startDate: event.target.value }))}
            />
            <Input
              label="End Date"
              type="date"
              value={generateForm.endDate}
              onChange={(event) => setGenerateForm((current) => ({ ...current, endDate: event.target.value }))}
            />
          </div>
          ...
          <Select
            ...
            onChange={(event) => setGenerateForm((current) => ({ ...current, staffId: event.target.value }))}
            options={staffOptions}
          />

          <div style={{ display: 'flex', gap: '12px' }}>
            <Button onClick={handleGenerateVisits} disabled={generateSubmitting}>
              {generateSubmitting ? 'Generating...' : 'Generate Visits'}
            </Button>
          ...
          {generateResult && (
            <div style={{ padding: '16px', borderRadius: '12px', backgroundColor: 'var(--color-background-secondary)' }}>
              <div style={{ fontSize: '14px', fontWeight: 600, marginBottom: '8px' }}>
                Generation Summary
              </div>
              <div style={{ fontSize: '13px', color: 'var(--color-text-secondary)', display: 'grid', gap: '4px' }}>
                <div>Created: {generateResult.visitsCreated ?? 0}</div>
                <div>Duplicates skipped: {generateResult.skippedDuplicates?.length ?? 0}</div>
                <div>Conflicts skipped: {generateResult.skippedConflicts?.length ?? 0}</div>
                <div>Manual services skipped: {generateResult.skippedNonAutoGenerated?.length ?? 0}</div>
              </div>
            </div>
          )}
        </div>
